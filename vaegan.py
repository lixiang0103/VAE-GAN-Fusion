...
class vaegan(object):
    ...
    def build_model_vaegan(self):
        [input_imgs1, input_imgs2] = tf.split(self.images, 2, 0)
        # normal distribution for reparameterization trick
        self.eps = tf.random_normal(shape=(int(self.batch_size / 2), self.latent_dim), mean=0.0, stddev=1.0)
        # normal distribution for generator
        self.zps = tf.random_normal(shape=(int(self.batch_size / 2), self.latent_dim), mean=0.0, stddev=1.0)
        # give a random weight
        #self.W = tf.constant(np.random.randint(0, 2,int(self.batch_size/2)*self.latent_dim),shape=[int(self.batch_size/2),self.latent_dim],dtype=tf.float32)
        # get the half feacture of the two pictures
        self.W_1 =  tf.ones(shape=(int(self.batch_size/2), self.latent_dim*0.5),dtype=tf.float32)
        self.W_2 = tf.zeros(shape=(int(self.batch_size/2), self.latent_dim*0.5), dtype=tf.float32)
        self.conW = tf.ones(shape=(int(self.batch_size / 2), self.latent_dim), dtype=tf.float32)
        self.W = tf.concat([self.W_1, self.W_2], 1)
        self.z_mean,self.z_sigm = self.Encode(self.images)
        [self.z_mean1, self.z_mean2] = tf.split(self.z_mean, 2, 0)
        [self.z_sigm1, self.z_sigm2] = tf.split(self.z_mean, 2, 0)
        self.z_mean = tf.add(tf.multiply(self.W, self.z_mean1), tf.multiply(tf.subtract(self.conW, self.W), self.z_mean2))
        self.z_sigm = tf.add(tf.multiply(self.W, self.z_sigm1), tf.multiply(tf.subtract(self.conW, self.W), self.z_sigm2))
        # the final latent vector z
        self.z_x = tf.add(self.z_mean, tf.sqrt(tf.exp(self.z_sigm))*self.eps)
        self.x_tilde = self.generate(self.z_x, reuse=False)
        # the final images
        self.l_x_tilde, self.De_pro_tilde = self.discriminate(self.x_tilde)
        # randomal generation
        self.x_p = self.generate(self.zps, reuse=True)

        self.l_x,  self.D_pro_logits = self.discriminate(input_imgs1, True)
        ...
        # preceptual loss(feature loss) NLLNormal(self, pred, target)
    ...
    def discriminate(self, x_var, reuse=False):
        ...
        with tf.variable_scope("discriminator") as scope:
            ...
            conv4= tf.reshape(conv4, [int(self.batch_size/2), -1])
    ...
    # the size of z_var 32 * 128
        with tf.variable_scope('generator') as scope:
            ...
            d2 = tf.nn.relu(batch_normal(de_conv(d2, output_shape=[int(self.batch_size/2), 8, 8, 256], name='gen_deconv2'), scope='gen_bn2', reuse=reuse))
            d3 = tf.nn.relu(batch_normal(de_conv(d2, output_shape=[int(self.batch_size/2), 16,16, 128], name='gen_deconv3'), scope='gen_bn3', reuse=reuse))
            d4 = tf.nn.relu(batch_normal(de_conv(d3, output_shape=[int(self.batch_size / 2), 32, 32, 64], name='gen_deconv4'),scope='gen_bn4', reuse=reuse))
            d5 = tf.nn.relu(batch_normal(de_conv(d4, output_shape=[int(self.batch_size/2), 64, 64, 32], name='gen_deconv5'),scope='gen_bn5', reuse=reuse))
            d6 = de_conv(d5, output_shape=[int(self.batch_size/2), 64, 64, 3], name='gen_deconv6', d_h=1, d_w=1)
    ...
    def Encode(self, x):
        ...
        with tf.variable_scope('encode') as scope:
            conv1 = tf.nn.relu(batch_normal(conv2d(x, output_dim=32, name='e_c1'), scope='e_bn1'))
            conv2 = tf.nn.relu(batch_normal(conv2d(conv1, output_dim=64, name='e_c2'), scope='e_bn2'))
            conv3 = tf.nn.relu(batch_normal(conv2d(conv2, output_dim=128, name='e_c3'), scope='e_bn3'))
            conv4 = tf.nn.relu(batch_normal(conv2d(conv3, output_dim=256, name='e_c4'), scope='e_bn4'))
            conv4 = tf.reshape(conv4, [self.batch_size, 256 * 4 * 4])
            #fc1 = tf.nn.relu(batch_normal(fully_connect(conv3, output_size=1024, scope='e_f1'), scope='e_bn4'))
            z_mean = fully_connect(conv4, output_size=100, scope='e_f2')
            z_sigma = fully_connect(conv4, output_size=100, scope='e_f3')
